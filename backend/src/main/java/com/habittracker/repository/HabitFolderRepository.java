...
public interface HabitFolderRepository extends JpaRepository<HabitFolder, Long> {
       ...
       List<HabitFolder> findByUserAndFolderType(@Param("userId") Long userId,
                     @Param("folderType") HabitFolder.FolderType folderType);
       ...
       /**
        * Find all folders for a user ordered by parent ID and name (for tree building)
        */
       @Query("SELECT f FROM HabitFolder f WHERE f.user.id = :userId ORDER BY f.parent.id ASC NULLS FIRST, f.name ASC")
       List<HabitFolder> findAllByUserIdOrderByParentIdAscNameAsc(@Param("userId") Long userId);
}